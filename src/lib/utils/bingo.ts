...
export function detectBingo(board: Board): BingoLine[] {
	...
	for (let row = 0; row < size; row++) {
		...
		if (rowIndices.every((i) => goals[i].completed)) { // eslint-disable-line security/detect-object-injection
	...
	for (let col = 0; col < size; col++) {
		...
		if (colIndices.every((i) => goals[i].completed)) { // eslint-disable-line security/detect-object-injection
	...
	if (diagonal1.every((i) => goals[i].completed)) { // eslint-disable-line security/detect-object-injection
		completedLines.push({ type: 'diagonal', indices: diagonal1 });
	}

	// Check diagonal (top-right to bottom-left)
	const diagonal2 = Array.from({ length: size }, (_, i) => i * size + (size - 1 - i));
	if (diagonal2.every((i) => goals[i].completed)) { // eslint-disable-line security/detect-object-injection
